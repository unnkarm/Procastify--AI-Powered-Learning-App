...
import { analyzeNoteWorkload, generateAdaptiveRoutine } from '../services/geminiService';
import { StorageService } from '../services/storageService'; // Added Import
import { Clock, CheckCircle, RefreshCw, CalendarCheck, PlayCircle, Plus, BrainCircuit, Settings, Coffee, Trash2 } from 'lucide-react';
...
const Routine: React.FC<RoutineProps> = ({ user, setUser, notes, setNotes, onStartTask }) => {
  ...
  const renderSchedule = () => (
      <div className="max-w-4xl mx-auto">
          {routineMeta && (
              <div className="mb-8 p-4 bg-discord-accent/10 border border-discord-accent/20 rounded-xl flex items-start gap-4">
                  <div className="mt-1">
                      <BrainCircuit className="text-discord-accent" size={24} />
                  </div>
                  <div>
                      <h4 className="font-bold text-white mb-1">AI Projection</h4>
                                  ...
                                  <div className="flex items-center gap-2 mb-1">
                                      <span className={`text-xs font-bold uppercase tracking-wider
                                          ${task.type === 'procastify' ? 'text-purple-400' : 
                                            task.type === 'break' ? 'text-green-400' : 'text-discord-accent'}
                                      `}>
